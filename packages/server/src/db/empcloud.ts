...
// DB's `api_keys` table. emp-rewards reads that SAME table to validate a key —
...
//
// emp-rewards gates by role (authorize()), so we only need the owner's role
 ...
 * yet (EmpCloud migration 078 not applied) so emp-rewards never 500s on auth.
 */
