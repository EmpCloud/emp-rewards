    ...
    return (
      <div className="flex h-64 items-center justify-center">
        <div className="text-center">
          ...
          <h2 className="mt-4 text-lg font-semibold text-gray-900">{tr("Nomination Submitted!")}</h2>
          <p className="mt-1 text-sm text-gray-500">
            {selectedNominee?.first_name} {selectedNominee?.last_name}  {tr("has been nominated")}
  ...
  return (
    <div className="mx-auto max-w-2xl space-y-6">
      ...
      <div className="flex items-center gap-3">
        ...
        <div>
          <h1 className="text-2xl font-bold text-gray-900">{tr("Submit Nomination")}</h1>
          <p className="mt-1 text-sm text-gray-500">{tr("Nominate a colleague for an award.")}</p>
        </div>
      </div>
      ...
      {programsLoading ? (
        ...
        !error && (
          <div className="rounded-lg border border-gray-200 bg-white p-12 text-center">
            ...
            <p className="mt-3 text-sm text-gray-500">
              ...
              {tr("There are no active nomination programs right now.")}
            </p>
            <Link
              to="/nominations"
        ...
        >
          {/* Program */}
          <div>
            <label className="block text-sm font-medium text-gray-700 mb-1.5">{tr("Program")}</label>
            <select
            ...
            >
              <option value="">{tr("Select a program...")}</option>
              {programs.map((p) => (
                ...
                <div className="flex items-start gap-3">
                  ...
                  <div className="min-w-0">
                    ...
                    {selectedProgram.description && (
                      <p className="mt-0.5 text-xs text-gray-600">{tr(selectedProgram.description)}</p>
                    )}
                    <div className="mt-2 flex flex-wrap items-center gap-x-4 gap-y-1 text-xs text-gray-600">
                      <span className="inline-flex items-center gap-1">
                        <Coins className="h-3.5 w-3.5 text-amber-500" />
                        <span className="font-medium text-amber-700">
                          {selectedProgram.points_awarded.toLocaleString(activeLocale())}  {tr("pts")}
                        </span>
            ...
            {limitReached && (
              <div className="mt-3 flex items-center gap-2 rounded-lg border border-amber-200 bg-amber-50 p-3 text-sm text-amber-800">
                ...
                {tr("You have used all your nominations for this program.")}
              </div>
            )}
          </div>
          ...
          {/* Nominee */}
          <div>
            <label className="block text-sm font-medium text-gray-700 mb-1.5">{tr("Nominee")}</label>
            {selectedNominee ? (
              ...
              <div className="relative z-10">
                ...
                <input
                  type="text"
                  placeholder={tr("Search colleagues by name or email...")}
                  value={nomineeSearch}
                  onChange={(e) => {
          ...
          <div>
            <div className="mb-1.5 flex items-center justify-between">
              <label className="block text-sm font-medium text-gray-700">{tr("Reason")}</label>
            ...
            <textarea
              ...
              rows={5}
              placeholder={tr("Describe what makes them deserving of this award — specific contributions, impact, and examples help reviewers.")}
